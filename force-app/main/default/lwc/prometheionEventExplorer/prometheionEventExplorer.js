...
export default class PrometheionEventExplorer extends LightningElement {
    ...
    generateMockEvents(stats) {
        ...
        for (let i = 0; i < Math.min(stats.totalEvents || 50, 100); i++) {
            ...
            const riskLevel = this.eventRiskLevels[eventType] || riskLevels[Math.floor(Math.random() * 4)];
    ...
    calculateRiskScoreFromLevel(level) {
        switch (level) {
            ...
            case 'HIGH': return 70 + Math.floor(Math.random() * 20);
            case 'MEDIUM': return 40 + Math.floor(Math.random() * 30);
            default: return Math.floor(Math.random() * 40);
        }
    }
    ...
    handleExportCSV() {
        ...
        const headers = ['ID', 'Event Type', 'Risk Level', 'Risk Score', 'User', 'Timestamp', 'Source IP', 'Description'];
