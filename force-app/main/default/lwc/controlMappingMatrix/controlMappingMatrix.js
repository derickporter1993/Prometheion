...
const CONTROL_MAPPINGS = {
    'NIST': {
        controls: [
            { id: '1', code: 'ID.AM-1', name: 'Asset Inventory', description: 'Physical devices and systems are inventoried' },
            { id: '2', code: 'ID.AM-2', name: 'Software Inventory', description: 'Software platforms and applications are inventoried' },
            { id: '3', code: 'PR.AC-1', name: 'Identity Management', description: 'Identities and credentials are managed' },
            ...
            { id: '6', code: 'DE.CM-1', name: 'Network Monitoring', description: 'The network is monitored' },
            { id: '7', code: 'RS.RP-1', name: 'Response Plan', description: 'Response plan is executed during incident' },
...
export default class ControlMappingMatrix extends LightningElement {
    ...
    handleSourceChange(event) {
        ...
        this.targetFrameworks = this.availableTargetFrameworks
            .slice(0, 3)
            .map(f => f.value);
        this.loadMappings();
    }
    ...
    loadMappings() {
        ...
        try {
            ...
            this.mappingRows = sourceData.controls.map(control => {
                ...
                const coveragePercent = this.targetFrameworks.length > 0
    ...
    getMappingIcon(type) {
        switch (type) {
            case 'direct': return 'utility:success';
            case 'partial': return 'utility:warning';
            default: return '';
        }
    }
    ...
    handleExport() {
        ...
        a.href = url;
        a.download = 'control_mapping_' + this.sourceFramework + '_' + new Date().toISOString().split('T')[0] + '.csv';
        a.click();
