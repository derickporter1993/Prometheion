...
## SentinelReasoningEngine.cls (P0/P1)
- `explainViolation` assumes `Sentinel_AI_Settings__c.getInstance()` is never null, so a missing settings row can cause a null dereference before `Enable_AI_Reasoning__c` is read. **Fix:** add a null-safe accessor that seeds defaults or fails fast with a clear error.
...
- `generateDeterministicHash` invoked from `logAdjudication` relies on `System.now()` inside `SentinelGraphIndexer`, making adjudication IDs non-deterministic and time-dependent. **Fix:** generate hashes only from stable record attributes (e.g., Id, LastModifiedDate) to prevent duplicate graph nodes.

## SentinelGraphIndexer.cls (P0/P1)
...
## SentinelComplianceScorer.cls (P1)
...
## SentinelAISettingsController.cls (P0)
...
1) **P0 Security/Integrity:** Harden `SentinelAISettingsController`, add null/ID stability fixes in `SentinelReasoningEngine` and `SentinelGraphIndexer`.
